...
(defn place-search [e! place-search]
  (let [results (:results place-search)]
    [:div.place-search
     ...
     [form-fields/field {:type :string :label (tr [:place-search :name])
                         :update! #(e! (ps/->SetPlaceName %))}
      (get place-search :name)]

     [napit/tallenna {:on-click #(e! (ps/->SearchPlaces))}
      (tr [:place-search :search-button])]
     ...
     [leaflet/Map {:center #js [65 25]
      ...
      (for [{namefin :namefin :as result} results]
        ^{:key namefin}
