...
(defn harja-bars [{:keys [width height label-fn value-fn key-fn color-fn color
...
(defn harja-pie [{:keys [width height radius show-text show-legend]} items]
  (let [hover (atom nil)
...
(defn monitor-main [e! {:keys [page monitor-data] :as app}]
  (let [bar-color "rgb(0, 170, 187)"
        bars-data [{:label "11/2018" :value 2342}
                   {:label "12/2018" :value 2804}
                   {:label "1/2019" :value 2830}]]
    #_[:svg {:width 200 :height 200} ;; svg testing
     [:g
      [:rect {:x 10 :y 10 :width 20 :height 100 :fill bar-color}
       [:title "bar 1"]]
      [:rect {:x 40 :y 10 :width 20 :height 60 :fill bar-color}
       [:title "bar 2"]]]]
    [:div 
     [harja-pie {:height 500 :width 600 :radius 140 :show-text false :show-legend true}
      {"Taksiliikenne" 100
       "Säännöllisen aikataulun mukainen liikenne" 20
       "Välityspalvelut" 5}]
     [harja-bars {:width 400 :height 300
                  :label-fn :label
                  :value-fn :value
                  :format-amount str
                  :ticks [0 1000 2000 3000]} bars-data]]))
