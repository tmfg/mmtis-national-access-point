...
psql -h napotedb -U napote postgres -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname='napote' AND pid <> pg_backend_pid();"
psql -h napotedb -U napote postgres -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname='napotetest' AND pid <> pg_backend_pid();"
psql -h napotedb -U napote postgres -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname='napotetest_template' AND pid <> pg_backend_pid();"

psql -h napotedb -U napote postgres -c "DROP DATABASE IF EXISTS napote;"
